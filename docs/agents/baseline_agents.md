...
- Endgame search now descends through opponent turns (modelled with the deterministic greedy baseline) to the next own turn, with a beam of four continuations. Before 2026-09-04 it stopped after one ply in every multiplayer game, so `search_depth` had no effect. Depth 2 on every turn is worth roughly +7 points per game against this roster; searching every turn rather than the last five cubes of a round adds about +6 more (`docs/experiments/search_depth_experiment.md`). With `determinization_samples=4` the search scores actions over resampled decks and opponent hands instead of the true state; the leak-free gain is +10.4 at depth 3 every turn (`docs/experiments/determinized_search_test.md`). Since 2026-09-06 the engine resolves feeder rolls when a gain-food action is applied and the samples resample `random_seed`, so the search sees rerolls as a chance node (`docs/experiments/reroll_chance_node.md`). `planning_horizon` selects how far the evaluator counts turns: `"round"` (default; remaining cubes this round) or `"game"` (cubes in every remaining round). The search trigger always counts round cubes. The game horizon costs −12.0 points with the current coefficients (`docs/experiments/game_horizon_ablation.md`): the round horizon is what tells the evaluator when to realize points. Below the root the search keeps at most `search_food_candidates` (6) gain-food continuations per node, ranked by expected demand-weighted food, because preference actions can list 70+ near-identical food choices at a dry feeder. It binds on 30% of decisions, cuts the worst decision from 1262 s to 491 s, and costs about 1 point per game (−0.99, p=0.074; win −0.044, p=0.048), so strength claims should use `search_food_candidates=None` (`docs/experiments/search_food_candidates.md`). Remaining gap: the opponent model is fixed to greedy.
